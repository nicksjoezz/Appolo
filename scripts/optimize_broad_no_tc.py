...
def optimize_broad_no_tc():
    ...
    sensitivities = [1, 2, 5, 10, 15, 18, 20]

    for s in sensitivities:
        print(f"Testing sensitivity {s}...", file=sys.stderr)
        # Using the best balanced filter EMA 500
        for el in [500]:
            for tp in [0.05, 0.1, 0.15]:
                ...
                if res['n_trades'] >= 10:
                    results.append({'sens': s, 'el': el, 'tp': tp, 'res': res})

    results.sort(key=lambda x: x['res']['total_return_pct'], reverse=True)

    print("\nTOP BROAD NO-TC RESULTS")
    print("="*60)
    for i, item in enumerate(results[:20]):
        r = item['res']
        print(f"{i+1}. Sens: {item['sens']} | EMA: {item['el']} | TP: {item['tp']:.0%}")
