...
def fetch_history(symbol, timeframe, years=3):
    ...
    while current_date <= end_date - timedelta(days=32):
        ...
        if df is not None:
            ...
            print(f"Monthly {current_date.year}-{current_date.month:02d} failed, breaking to daily.")
            break
...
if __name__ == "__main__":
    ...
    symbol = 'ROSEUSDT'
    # For the new strategy we definitely need 5m data for 3 years
    df_5m = fetch_history(symbol, '5m', years=3)
    if df_5m is not None:
        df_5m.to_csv('data/ROSEUSDT_5m_3y.csv')
