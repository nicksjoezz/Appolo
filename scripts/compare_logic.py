...
class OptimisticBacktester:
    ...
    def run_backtest(self, df, signals, tp_pct=0.02, sl_pct=0.01, margin_pct=0.1):
        ...
        for i in range(len(df)):
            ...
            # OPTIMISTIC: Check exit only AFTER the bar has closed (ignoring the H/L of the entry bar itself)
            if self.position != 0:
                exit_price = None
                if self.position == 1:
                    if current_low <= self.sl: exit_price = self.sl
                    elif current_high >= self.tp: exit_price = self.tp
                else:
                    if current_high >= self.sl: exit_price = self.sl
                    elif current_low <= self.tp: exit_price = self.tp
                if exit_price:
                    if self._close_pos(exit_price, current_time, "Exit"): break

            # Signal on bar i, Entry on open of i+1
...
if __name__ == "__main__":
    ...
    signals = imba_algo_trend_filtered(df, sensitivity=18, ema_filter=False, rsi_filter=False)

    # Run Optimistic
    opt = OptimisticBacktester()
    res_opt = opt.run_backtest(df, signals, tp_pct=0.05, sl_pct=0.01, margin_pct=0.1)
    print(f"Optimistic Final Balance: {res_opt:.2f}")

    # Run Realistic (using the current backtester script)
    from backtester import Backtester
    real = Backtester()
    res_real = real.run_backtest(df, signals, tp_pct=0.05, sl_pct=0.01, margin_pct=0.1)
    print(f"Realistic Final Balance: {res_real['final_balance']:.2f}")
